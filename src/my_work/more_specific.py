...
class Encoder(nn.Module):
    def __init__(self, bert_dim, intermediate_dim, final_dim):
        ...
        self.conv1d = nn.Conv1d(in_channels=60 + 256, out_channels=intermediate_dim, kernel_size=1)
...
class DualDecoderModel(nn.Module):
    def __init__(self, bert_dim, intermediate_dim, encoder_output_dim, num_classes):
        ...
        concatenated_dim = bert_dim * 2
        self.encoder = Encoder(concatenated_dim, intermediate_dim, encoder_output_dim)
        self.polarity_decoder = Decoder(encoder_output_dim, intermediate_dim, encoder_output_dim // 2)
        self.polarity_free_decoder = Decoder(encoder_output_dim, intermediate_dim, encoder_output_dim // 2)
        self.polarity_classifier = PolarityClassifier(encoder_output_dim // 2, num_classes)
    ...
    def forward(self, bert1, bert2):
        ...
        return encoded, polarity_decoded, polarity_free_decoded, polarity_pred

# Losses
def compute_losses(encoder_output, polarity_decoded, polarity_free_decoded, polarity_pred, polarity_labels):
    # Reconstruction loss
    ...
    confusion_loss = 1.0 / (F.binary_cross_entropy_with_logits(polarity_pred.float(), polarity_labels.float()) + 1e-8)
...
def train(model, dataset, optimizer, path_to_data, embedding_batch_num, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for bert1, bert2, polarity_labels in data_loader:
            ...
            # Forward pass
            encoded, polarity_decoded, polarity_free_decoded, polarity_pred = model(bert1, bert2)

            # Loss computation
            recon_loss, class_loss, conf_loss = compute_losses(
                encoded, polarity_decoded, polarity_free_decoded, polarity_pred, polarity_labels
            )
