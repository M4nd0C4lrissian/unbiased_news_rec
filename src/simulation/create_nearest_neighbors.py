...
landmarks = pd.read_csv("unbiased_news_rec\\src\data\\landmark_data\\landmark_embeddings.csv")
...
def landmark_embedding(landmarks, embedding):
    return [dist(lm, embedding) for lm in landmarks]
...
for i in range(len(classes)):
    ...
    for j in range(class_users.shape[0]):
        ...
        user_embedding = generate_user_content_embedding(topic_lists, data_source_path, user_utility, encoder_output_dim, encoder, polarity_decoder)
        
        new = landmark_embedding(landmarks, user_embedding)
        
        distance_embeddings[j] = new
