...
def evaluate(out_path, correlation_matrix, all_weights, topk):
    ...
    for u in range(user_item_matrix.shape[0]):
        ...
        predicted_row = copy.deepcopy(predicted_rating_matrix[u]).cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    weight_paths = ['FN_rating_target_h_5_per_user.csv','NN_rating_target_h_5_per_user.csv']
    top = [False, True]
    paths = ['FN_Rating_CPC.csv', 'NN_Rating_CPC.csv']
